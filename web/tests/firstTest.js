...
(async function example() {
  ...
  try {
    ...
    //Name
    var element1 = await driver.findElement(
      By.css('[placeholder="Enter event name"]'),
    );
    ...
    //Address
    var element3 = await driver.findElement(
      By.css('[placeholder="Enter event address"]'),
    );
    element3.sendKeys('Test Addr');

    //Description
    var element4 = await driver.findElement(
      By.css('[placeholder="Enter event description (optional)"]'),
    );
    element4.sendKeys('Test Desc');

    //Category
    var element5 = await driver.findElement(
      By.className(
    ...
    await driver
      .findElement(
        By.className(
          'items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2 ml-auto flex space-x-2',
        ),
      )
      .click();
  } finally {
